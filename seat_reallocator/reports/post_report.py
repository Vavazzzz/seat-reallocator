...
Merge logic:
  ...
  2. Inner-join DF2 on Sigillo — DF2 becomes the base (latest data),
...
def build(
    ...
    sheets = pd.read_excel(
        annotated_path,
        sheet_name=None,
        dtype={'Codice ordine': str, 'Sigillo': str},
    )
    ...
    realloc_cols = ['Sigillo', 'Nuovo posto', 'Stato']
    ...
    df1_slim = df1_filtered[realloc_cols].drop_duplicates(subset=['Sigillo'])
    ...
    if 'Sigillo' not in df2.columns:
        sys.exit("DF2 is missing the 'Sigillo' column.")
    ...
    merged = df2.merge(df1_slim, on='Sigillo', how='inner')
    print(f'  After DF1+DF2 merge: {len(merged):,} rows.', flush=True)
    if merged.empty:
        sys.exit('No matching rows after merging DF1 and DF2 on Sigillo.')
