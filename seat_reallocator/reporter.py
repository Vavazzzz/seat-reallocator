...
def write_full_report(
    ...
    """
    Annotate every row of the source file with 'Nuovo posto' and 'Stato', then
    write one sheet per event to an xlsx file.

    If the source already has 'Stato'/'Nuovo posto'/'Nuova fila' columns (e.g. a
    previously-annotated file), their values are used as the baseline and only the
    rows affected by this run's moves are updated.
    ...
    # Use existing column values as baseline when the source is already annotated;
    # otherwise initialise to defaults so every row has a value.
    # Always coerce numeric columns to Int64 here so later assignments are type-compatible.
    ...
    if has_cross_row:
        if 'Nuova fila' not in big_df.columns:
    ...
    # Drop temp columns, reinsert Nuovo posto + Stato (+ Nuova fila) right after Posto
